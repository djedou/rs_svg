...
use std::rc::Rc;
use super::ellipse_attributes::EllipseProps;
...
impl Component for SvgEllipse {
    ...
    fn view(&self) -> Html {
        ...
        let mut line_tag = html! {<ellipse />};

        set_attribute(self.cx.as_ref(), &mut line_tag, "cx");
        set_attribute(self.cy.as_ref(), &mut line_tag, "cy");
        set_attribute(self.rx.as_ref(), &mut line_tag, "rx");
        set_attribute(self.ry.as_ref(), &mut line_tag, "ry");
